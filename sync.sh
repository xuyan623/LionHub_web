...
# Backup runtime database (contains live data from phone usage)
if [ -f "$DB_FILE" ]; then
...
# Fetch and force overwrite to match GitHub
git fetch origin
git reset --hard origin/master
echo "[✓] Code synced from GitHub"

# Restore runtime database (preserve phone-side live data)
